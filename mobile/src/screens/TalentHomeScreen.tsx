...
import { formatTalentAvailabilityLabel, parseTalentAvailabilityFromBio } from "../lib/talentAvailability";
...
function getDisplayName(fullName: string | null, prenom: string | null, nom: string | null) {
...
function isActiveOffer(status: string | null) {
  if (!status) return true;
  const normalized = status.trim().toLowerCase();
  return ["active", "actif", "published", "ouverte", "open"].includes(normalized);
}
...
function formatSalary(min: number | null, max: number | null) {
  if (typeof min === "number" && typeof max === "number") return `${min} - ${max} EUR`;
  if (typeof min === "number") return `A partir de ${min} EUR`;
  if (typeof max === "number") return `Jusqu'a ${max} EUR`;
...
export function TalentHomeScreen() {
  ...
  const fetchDashboard = useCallback(async () => {
    ...
    try {
      const [offersResult, applicationsResult, messagesResult, docsResult] = await Promise.all([
        supabase
          .from("offres")
          .select(
      ...
      const offersData = ((offersResult.data ?? []) as OffreRow[]).filter((offer) => isActiveOffer(offer.statut));
      const applicationsData = (applicationsResult.data ?? []) as CandidatureRow[];
      ...
      const offerMap = new Map(
        offersData.map((offer) => [
          offer.id,
          { title: offer.titre || "Offre", entrepriseId: offer.entreprise_id || "" },
        ]),
      );

      const missingOfferIds = applicationsData
        .map((application) => application.offre_id)
        .filter((id) => id && !offerMap.has(id));
      ...
      if (missingOfferIds.length > 0) {
        ...
        ((fallbackOffers.data ?? []) as Array<{ id: string; titre: string; entreprise_id: string }>).forEach(
          (offer) => {
            offerMap.set(offer.id, {
              title: offer.titre || "Offre",
              entrepriseId: offer.entreprise_id || "",
            });
          },
        );
      }
      ...
      const appsMapped: AppItem[] = applicationsData.map((application) => ({
        ...
        entrepriseId: offerMap.get(application.offre_id)?.entrepriseId || "",
        offreTitre: offerMap.get(application.offre_id)?.title || "Offre",
      ...
      setStats({
        offresDisponibles: offersData.length,
        candidaturesEnvoyees: applicationsData.length,
        candidaturesAcceptees: applicationsData.filter((application) => application.statut === "acceptee").length,
        candidaturesRefusees: applicationsData.filter((application) => application.statut === "refusee").length,
        messagesNonLus: messagesData.filter((message) => message.destinataire_id === user.id && !message.lu).length,
        documentsDemandes: docsData.filter((doc) => doc.status === "requested").length,
      });
    } catch {
      setErrorMessage("Impossible de charger vos donnees pour le moment.");
    }
  }, [user]);
  ...
  const sendConversationReply = useCallback(async () => {
    ...
    if (!application || !application.entrepriseId) {
      ...
      .insert([
        {
          ...
          destinataire_id: application.entrepriseId,
          contenu: content,
          lu: false,
        },
      ])
    ...
    setConversationSending(false);
    setActionMessage("Message envoye a l'entreprise.");
  }, [applications, conversationDraft, selectedConversationCandidatureId, user]);

  const responseRate = useMemo(() => {
    if (stats.candidaturesEnvoyees === 0) return 0;
    const treated = stats.candidaturesAcceptees + stats.candidaturesRefusees;
    return Math.round((treated / stats.candidaturesEnvoyees) * 100);
  }, [stats]);
  ...
  const availabilityLabel = useMemo(() => {
    const parsed = parseTalentAvailabilityFromBio(profile?.bio);
    const formatted = formatTalentAvailabilityLabel(parsed.type, parsed.detail);
    return formatted || "A preciser";
  }, [profile?.bio]);

  const profilePosition = profile?.poste?.trim() || "Poste a preciser";
  const profileSector = profile?.secteur?.trim() || "Secteur a preciser";

  if (loading) {
    return (
      <View style={styles.loadingContainer}>
        <Text style={styles.loadingText}>Chargement de votre espace talent...</Text>
      </View>
    );
  }
    ...
    >
      <View style={styles.hero}>
        <Text style={styles.title}>
          Bonjour {getDisplayName(profile?.full_name ?? null, profile?.prenom ?? null, profile?.nom ?? null)}
        </Text>
        <Text style={styles.subtitle}>
          Votre espace mobile talent: candidatures, messages et documents sans perdre de temps.
        </Text>
      </View>
      ...
      {activeTab === "apercu" && (
        <>
          <View style={styles.profileSummaryCard}>
            <Text style={styles.profileSummaryTitle}>Profil visible par les entreprises</Text>
            <Text style={styles.profileSummaryMeta}>Poste: {profilePosition}</Text>
            <Text style={styles.profileSummaryMeta}>Secteur: {profileSector}</Text>
            <Text style={styles.profileSummaryMeta}>Disponibilite: {availabilityLabel}</Text>
          </View>

          <View style={styles.statsGrid}>
            <View style={styles.statCard}>
              <Text style={styles.statLabel}>Offres disponibles</Text>
              <Text style={styles.statValue}>{stats.offresDisponibles}</Text>
            </View>
            <View style={styles.statCard}>
              <Text style={styles.statLabel}>Candidatures envoyees</Text>
              <Text style={styles.statValue}>{stats.candidaturesEnvoyees}</Text>
            </View>
            <View style={styles.statCard}>
          ...
          <View style={styles.section}>
            ...
            <Text style={styles.quickActionItem}>Documents demandes: {stats.documentsDemandes}</Text>
            <Text style={styles.quickActionItem}>Candidatures acceptees: {stats.candidaturesAcceptees}</Text>
      ...
      {activeTab === "offres" && (
        <View style={styles.section}>
          ...
          {offers.length === 0 ? (
            <Text style={styles.emptyText}>Aucune offre visible pour le moment.</Text>
          ) : (
            offers.map((offer) => (
          ...
          {selectedOffer ? (
            <View style={styles.detailPanel}>
              ...
              <Text style={styles.cardMeta}>Salaire: {formatSalary(selectedOffer.salaire_min, selectedOffer.salaire_max)}</Text>
          ...
          {applications.length === 0 ? (
            ...
            applications.map((application) => (
              <View key={application.id} style={styles.card}>
                ...
                <Text style={styles.cardMeta}>Envoyee le {formatDate(application.createdAt)}</Text>
      ...
      {activeTab === "documents" && (
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>Documents demandes</Text>
          {documents.length === 0 ? (
